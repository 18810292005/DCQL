...
class MGEException(Exception):
    """
    异常类，必须指定产生的异常的MGEError枚举值
    """
    _code_set = set()
    ...
    def __init__(self, msg: str = '', code: int = 500, status_code: int = 422, data=None):
        self._msg = str(msg)  # ugettext_lazy
        self._code = code
    ...
    def to_dict(self):
        d = {'msg': self._msg, 'code': self._code, 'data': self._data}
    ...
    def __eq__(self, other: 'MGEException'):
        if not isinstance(other, MGEException):
            return False
        return self._code == other._code

    @property
    def full_string(self):
        msg = f"MGEException: {self._msg} "
        if self._error_detail:
            msg += f" ({self._error_detail})"
        if self._secret_detail:
            msg += f'\n{self._secret_detail}'
...
class _MGEErrorMetaClass(type):
    _error_code_dict = {}
    _enum_members = set()

    def __new__(mcs, cls_name, bases, class_dict):
        for member, value in class_dict.items():
            if isinstance(value, MGEException):
                if value.code in _MGEErrorMetaClass._error_code_dict:
                    raise ValueError(f"错误码{value.code}已经存在({_MGEErrorMetaClass._error_code_dict[value.code][0]})")
                _MGEErrorMetaClass._error_code_dict[value.code] = member, value
                _MGEErrorMetaClass._enum_members.add(member)
        return super().__new__(mcs, cls_name, bases, class_dict)

    def __getattribute__(self, item):
        super_item = super().__getattribute__(item)
        if item in _MGEErrorMetaClass._enum_members:
            return MGEException(msg=super_item.msg, code=super_item.code, status_code=super_item.status_code)
        return super_item

    def __setattr__(self, key, value):
        raise ValueError("Cannot set attributes to MGEError")

    def __getitem__(self, item: int) -> MGEException:
...
class MGEError(metaclass=_MGEErrorMetaClass):
    """
    错误枚举
    ...
    USER_NOT_FOUND = e(_("User not found"), 2003, 0)
    ...
    NO_VALID_FILE = e(_("Cannot find any xlsx, json or xml file."), 2108, 422)
    ...
    TEMPLATE_REFERENCED = e(_("T"), 2300, )
    ...
    DOI_VERIFICATION_ABNORMAL = e(_("DOI file verification failed"), 5001, 422)
    DOI_UNQUALIFIED = e(_("DOI file unqualified"), 5002, 422)
    DOI_ALREADY_EXISTS = e(_("DOI already exists"), 5003, 422)
    DOI_REGISTER_STATE_ERROR = e(_("DOI application has already been passed or rejected."), 5005, 422)
    DOI_DATA_DELETE_ERROR = e(_("data DOI is not null, and can not be deleted"), 5006, 422)
    DATASET_ALREADY_EXISTS = e(_("Dataset already exists"), 5007, 422)
    DATASET_REPEAT_REFERENCE = e(_("Dataset are referenced repeatedly"), 5008, 422)
    BAD_DOI = e(_("Doi is not correct"), 5009, 422)
    ...
    PARSE_ERROR = e(_("Can not parse query"), 7006, 500)
