...
class HttpTools:
    ...
    def get_oauth2_token(cls):
        """
        need login_as_root_user first
        :return: oauth token
        """
        if cls.token is not None:
            return cls.token
        user = User.objects.get(username='supertest')
        application = create_oauth2_application(user, 'hello', 'http://localhost')
        cls.token = create_oauth2_access_token(user, application).token
        return cls.token
