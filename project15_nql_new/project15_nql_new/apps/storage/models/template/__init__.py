...
from apps.storage.models import data as data_models  # 防止循环引用
from apps.storage.models.material import MaterialCategory, MaterialMethod
...
class TemplateFieldEnum(IntEnum):
    ...
    @property
    def is_es_in_array(self, without: List['TemplateFieldEnum'] = None):
        nested = [
            TemplateFieldEnum.TABLE,
            TemplateFieldEnum.ARRAY
        ]
        if without is not None:
            nested = list(set(nested).difference(set(without)))
        return self in nested
    ...
    def validate_content(content: dict):
        ...
        def _validate_field(field_dict):
            if 't' not in field_dict:
                raise ValueError('Key \"t\" is missing')
            ...
            if field_type.is_choice:
                ...
                if not (options or groups):  # 不同时为空
                    raise ValueError('At lease one option or group should be set.')
                if not isinstance(options, (tuple, list)):
                    raise ValueError('opt should be list or tuple.')
                if groups:
                    if isinstance(groups, (list, tuple)):
                        for grp in groups:
                            if isinstance(grp, dict) and 'name' in grp and 'items' in grp and isinstance(grp['items'],
                                ...
                                if not grp['items']:
                                    raise ValueError(
                                        "Group %s's items is empty" % grp['name'])
                            else:
                ...
                if '_head' not in misc:
                    raise ValueError('Key \"_head\" is missing')
                if not isinstance(misc['_head'], (list, tuple)):
                    raise ValueError('\"_head\" must be a list or tuple')
                for head_name in misc['_head']:
                    if head_name not in misc:
                        raise ValueError('Field "%(header)s" is missing from "%(field)s"' % {'header': head_name,
                                                                                             'field': str(misc)})
                ...
                if '_ord' not in misc:
                    raise ValueError('Key \"_ord\" is missing')
                if not isinstance(misc['_ord'], (list, tuple)):
                    raise ValueError('\"_ord\" must be a list or tuple')
                for sub_field_name in misc['_ord']:
                    if sub_field_name not in misc:
                        raise ValueError('Field "%(sub_field_name)s" is missing from "%(field)s"' % {
                ...
                if '_opt' not in misc:
                    raise ValueError("Key \"_opt\" is missing")
                if not isinstance(misc['_opt'], (list, tuple)):
                    raise ValueError('\"_opt\" must be a list or tuple')
                ...
                if misc['type'] not in scope:
                    raise ValueError("Key \"type\" is out of range")
...
class Template(models.Model):
    ...
    tag = models.IntegerField(default=TemplateTag.ORDINARY, verbose_name="模板标签",
                              choices=TemplateTag.choices(), null=True, blank=True)

    class Meta:
        ordering = ('-pub_date',)

    def save(self, check_pk=True, *args, **kwargs):
        if not self.category.leaf:
            raise MGEError.PERMISSION_DENIED("该类别并非叶子类别")
        if self.pk and check_pk:
            this = Template.objects.get(id=self.pk)
            if this.category != self.category:
                with transaction.atomic():
                    data_models.DataMeta.objects.filter(template=self).update(category=self.category)
    ...
    def review_status_description(self):
        return data_models.DataReviewState(self.review_state).description
    ...
    @property
    def pending(self):
        return self.review_state == data_models.DataReviewState.PENDING

    @property
    def approved(self):
        return self.review_state == data_models.DataReviewState.APPROVED

    @property
    def disapproved(self):
        return self.review_state == data_models.DataReviewState.DISAPPROVED
    ...
    def approve(self, reviewer: User):
        with transaction.atomic():
            ...
            #     h.save()
            Template.objects.filter(pk=self.pk).update(review_state=data_models.DataReviewState.APPROVED,
                                                       reviewer=reviewer)

    def disapprove(self, reviewer: User, reasons: List[data_models.DisapproveReason], reason: str):
        if reasons:
            reasons = [data_models.DisapproveReason(x).to_string for x in reasons]
        ...
        with transaction.atomic():
            ...
            #       h.meta_id_list.remove(self.id)
            #     h.save()
            Template.objects.filter(pk=self.pk).update(review_state=data_models.DataReviewState.DISAPPROVED,
    ...
    @property
    def ref_count(self):
        self._ref_count_cached = data_models.DataMeta.objects.filter(template=self).count()
        return self._ref_count_cached

    @property
    def ref_count_cached(self):
        if self._ref_count_cached is None:
    ...
    def add_data(self, meta_dict: dict, content_dict: dict, uploaded_by: str,
                 importing=False, check_uploaded_files=False) -> data_models.DataMeta:
        """
        唯一的添加数据的方法
    ...
    def modify_data(self, meta: data_models.DataMeta, meta_dict: dict, content_dict: dict, only_content=False):
        """
        唯一的修改数据的方法
        :param meta_dict: 包含元数据的dict
        :param content_dict:  包含数据内容的dict（必须是正确的最终格式，文件型和图片型已转为url）
    ...
    def filter_fields(self, excluded_fields=(), included_fields=()):
        if excluded_fields and included_fields:
            raise ValueError("Cannot specify excluded_fields and included_fields at the same time.")
        if not excluded_fields and not included_fields:
            raise ValueError("Must specify fields to exclude or fields to include.")
        ...
        if self.ref_count > 0:
            def compare(old, new, field_path_list=()):
                ...
                if old_t != new_t:
                    ...
                    raise ValueError(
                        _('Wrong field type: field "%(field)s" should be %(should)s, not %(should_not)s.' % {
                            'field': '.'.join(field_path_list), 'should': old_t.description,
                            'should_not': new_t.description
                        }))
...
class MaterialMethodTemplateRelation(models.Model):
    template = models.OneToOneField(Template, on_delete=models.CASCADE, related_name='method_rel')
    method = models.ForeignKey(MaterialMethod, on_delete=models.CASCADE)  # 如果MaterialMethod删除了，这个关系也就不存在了
