    ...
    def write_data(self, data_meta: DataMeta):
        ...
        def _add_field(field_meta: TemplateField, field_value, header_path_list: tuple):
            ...
            if field_meta.field_type.is_array:
                ...
                if field_value:
                    rows = DataFieldTableRow.objects(_owner_id=field_value)  # TODO 找不到
                    for index, row in enumerate(rows):
                        for table_header in field_meta.sub_fields:
                            _add_field(table_header, getattr(row, table_header.field_name, None),
                ...
                if field_value:
                    container_content = DataFieldContainer.objects.get(id=field_value)
        ...
        for field in self._template.fields:
            _add_field(field, getattr(data_content, field.field_name, None), (field.field_name,))
