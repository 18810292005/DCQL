...
class TaskType(IntEnum):
    ...
    def result_to_html(self, result_dict):
        if self == TaskType.DATA_EXPORT or self == TaskType.EXPORT_FOR_CERTIFICATE:
            text = ugettext("Click To Download")
            return f"""
            ...
            return ugettext("%(data_count)s data") % {'data_count': result_dict['data_count']}
            ...
            return ugettext("<a href=\"%(url)s\">Click to view the certificate</a>") % {
...
class UserTask(models.Model):
    ...
    def waiting_count_description(self) -> str:
        ...
        if count == 0:
            return _("Server is busy at the moment")
