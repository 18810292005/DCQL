...
class TestDatabaseContribution(MGETestCase):
    ...
    def sync_category(self):
        """
         ...
         TODO：同步删除
        """
        categories = MaterialCategory.public_objects.all()
    ...
    def test_category_hidden(self):
        ...
        self.assertEqual(MaterialCategory.objects.all().count(), 3)  # material_categories_to_create() 中有一个分类被hidden
        db_contributions = DatabaseContribution.objects.all()
        for index, db_contribution in enumerate(db_contributions):
            material_category = MaterialCategory.public_objects.filter(id=db_contribution.category_id)
