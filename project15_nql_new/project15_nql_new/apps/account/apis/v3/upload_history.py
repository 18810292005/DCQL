...
from apps.storage.models.data import DataReviewState, UploadHistory, DataMeta
...
def upload_history(request: HttpRequest):
    ...
    for history in histories:
        ...
        if not data_meta:
            continue
        is_accepted = Acceptance.objects.filter(
            Q(ps_id=data_meta.get_project_id()) | Q(ps_id=data_meta.get_subject_id())).filter(
            state=AcceptanceState.FINISHED).count() > 0
        ret = {'id': history.id, 'upload_time': history.time, 'data_count': history.count,
               ...
               'title': history.title,
               'platform_belong': history.get_platform_belong_display(),
...
def public_upload_history(request: HttpRequest):
    ...
    try:
        ...
        is_accepted = Acceptance.objects.filter(
            ...
            state=AcceptanceState.FINISHED).count() > 0
        if is_accepted:
            raise MGEError.BAD_DATA("此数据已完成验收,无法修改公开时间")
