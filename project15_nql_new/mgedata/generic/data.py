...
from mgedata.generic.data_fields import DataField, ContainerField


class Data:
    attributes = ('title', 'doi', 'keywords', 'abstract', 'purpose', 'contributor',
                  'source', 'methods', 'reference', 'project', 'subject', 'project_info', 'subject_info',
                  ...
                  'other_info', 'content', 'public_date', 'public_range')
    ...
    field_checker = (
        ...
        (lambda x: isinstance(x, list) and all(map(lambda xi: isinstance(xi, str), x)),
         ('keywords',)),
        (lambda x: isinstance(x, list) and (set(x) & {'computation', 'experiment', 'production'}) == set(x),
         ('methods',)),
        (lambda x: x in ('self-production', 'reference'),
         ('source',)),
