...
from apps.storage.models import MaterialCategory, Template, MaterialProject
...
class TestGetMaterialCategory(MGETestCase):
    ...
    def postSetUp(self):
        self.category_public = MaterialCategory.objects.get(name_zh='leaf')
        self.category_private = MaterialCategory.objects.get(name_zh='leaf_private')
    ...
    def test_login(self):
        ...
        AuthorityOfPrivateMaterialCategory.objects.create(user=self.user,
                                                          category=MaterialCategory.objects.get(name_zh='middle_leaf'))
        AuthorityOfPrivateMaterialCategory.objects.create(user=self.user,
                                                          category=MaterialCategory.objects.get(name_zh='middle'))
...
class TestOauth2MaterialInformation(TestCase):
    ...
    def setUp(self) -> None:
        ...
        create_material_category_tree(MaterialCategory,
                                      [
...
class TestOauth2DataUpload(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.user = User.objects.get(pk='test')
        self.category_leaf = MaterialCategory.objects.get(name_en='leaf')
        self.application = create_oauth2_application(self.user, 'hello', 'http://localhost')
        self.access_code = create_oauth2_access_token(self.user, self.application)
