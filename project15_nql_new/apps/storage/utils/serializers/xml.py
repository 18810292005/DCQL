...
from apps.storage.models import DataContentImage, DataContentFile
...
class XMLHandler(DataHandler):
    ...
    def write_data(self, data_meta: DataMeta, with_none=True):
            ...
            if t.is_table:
                ...
                for sub_field in field_meta.sub_fields:
                    if sub_field.field_name in container_content:
                        sub_node = _add_field(sub_field, getattr(container_content, sub_field.field_name, None))
                        if sub_node is not None:
        ...
        for field in self._template.fields:
            node = _add_field(field, getattr(content, field.field_name, None))
            if node is not None:
        ...
        for index in range(1, number_of_data + 1):
            ...
            for field in DataMetaFieldEnum.reader_iterator(with_id=True):
                ...
                if field == DataMetaFieldEnum.DATA_ID:
                    ...
                    field_node.text = "MGE-SOURCE_HEADER v1 000 10 #"
                elif field == DataMetaFieldEnum.OTHER_INFO:
                    field_node.text = "project: 2016YFB0700500；subject: 2016YFB0700503"
                else:
