...
from apps.storage.docs.data import DataContent, DataFieldContainer, DataFieldTableRow
from apps.storage.models.data import DataMetaFieldEnum, DataMeta, DataPublicRange
        ...
        if TemplateFieldEnum.is_range_error(field_meta.range_type):
            try:
                ...
                raise ParsingError(ParsingErrorEnum.INVALID_VALUE, o1=raw_value,
                                   o2=ugettext("Error Type"), o3=field_meta.external_field_path_str)
        else:
            try:
                ...
                raise ParsingError(ParsingErrorEnum.INVALID_VALUE, o1=raw_value,
                                   o2=ugettext("Interval Type"), o3=field_meta.external_field_path_str)
...
class ExcelHandler(DataHandler):
    ...
    def _create_sheet(self, title=None, field_meta: TemplateField = None):
        ...
        if title is None or title == self._root_data_name:
            ...
            ws.cell(row=Cursor.DATA_START_ROW_NUM - 1, column=1).value = ugettext('Data ID')
        else:
    ...
    def _create_headers_from_field(self, sheet, field_meta: TemplateField, fill_header=True,
        ...
        if t.is_table:
            ...
            if in_array:
                ws.cell(row=Cursor.DATA_START_ROW_NUM - 1, column=2).value = ugettext(
                    'Item Index')  # sheet的第二列是Item Index
            ...
            ws.cell(row=Cursor.DATA_START_ROW_NUM - 1, column=2).value = ugettext('Item Index')  # 第二列是Item Index
            if not (element_t.is_container or element_t.is_generator):
                ws.cell(row=Cursor.DATA_START_ROW_NUM - 1, column=3).value = ugettext('Items')  # 第三列是Items
            self._sheet_name_cursor_map[ws.title].move_right()
    ...
    def _write_reference_table(self, ws, column):
        data = DataMeta.OtherInfoHelper.subjects_representation_group_by_project_name()
        for _ in range(column):
            self._reference_cursor.move_right()
        for project_name in data:
            for j, subject_representation in enumerate(data[project_name]):
                ws.cell(**self._reference_cursor).value = subject_representation
    ...
    def _create_meta_sheet(self):
        ...
        for index, field in enumerate(iterator):
            cursor.move_right()
            ws.cell(row=cursor.row, column=cursor.column).value = field.description
            if field == DataMetaFieldEnum.SOURCE:
                if self._mode != DataMode.WRITE:
                    source_dv = DataValidation(type="list", formula1='"' + ','.join(
                        [ugettext(x) for x in DataMeta.SourceHelper.SOURCE_TUPLE]) + '"', allow_blank=True)
                    column_letter = get_column_letter(cursor.column)
                    source_dv.ranges.add(f'{column_letter}2:{column_letter}1048576')
                    ws.add_data_validation(source_dv)

                for method in DataMeta.SourceHelper.METHOD_TUPLE:
                    cursor.move_right()
                    ws.cell(row=cursor.row, column=cursor.column).value = ugettext(method)
                    if self._mode != DataMode.WRITE:
                        dv = DataValidation(type="list",
                                            formula1='"' + ','.join((ugettext('Yes'), ugettext('No'))) + '"',
                                            allow_blank=True)
                        column_letter = get_column_letter(cursor.column)
                        dv.ranges.add(f'{column_letter}2:{column_letter}1048576')
                        ws.add_data_validation(dv)
            if field == DataMetaFieldEnum.OTHER_INFO:
                ws.cell(row=cursor.row, column=cursor.column).value = ugettext(DataMeta.OtherInfoHelper.FIELDS[1])
                if self._mode != DataMode.WRITE:
                    dv = DataValidation(type="list",
                                        formula1="=${}${}:${}${}".format(
            ...
            if field == DataMetaFieldEnum.PUBLIC_DATE:
                dv = DataValidation(type="list",
                                    formula1='"' + ",".join([f'{time_option}年后公开' for time_option in
                                                             DataPublicRange.public_time]) + '"')
                column_letter = get_column_letter(cursor.column)
                dv.ranges.add(f'{column_letter}2:{column_letter}1048576')
                ws.add_data_validation(dv)

            if field == DataMetaFieldEnum.PUBLIC_RANGE:
                dv = DataValidation(type="list",
                                    formula1='"' + ",".join([option for option in
                                                             DataPublicRange.public_range_zh]) + '"')
                column_letter = get_column_letter(cursor.column)
                dv.ranges.add(f'{column_letter}2:{column_letter}1048576')
                ws.add_data_validation(dv)
    ...
    def write_data(self, data_meta: DataMeta):
        ...
        for index, field in enumerate(DataMetaFieldEnum.writer_iterator()):
            # 填写其他元数据
            if field == DataMetaFieldEnum.SOURCE:
                ws.cell(**self._meta_cursor).value = data_meta.get_source()
                for method in DataMeta.SourceHelper.METHOD_TUPLE:
                    self._meta_cursor.move_right()
                    if method in data_meta.get_methods():
                        ws.cell(**self._meta_cursor).value = ugettext("Yes")
                    else:
                        ws.cell(**self._meta_cursor).value = ugettext("No")
            elif field == DataMetaFieldEnum.OTHER_INFO:
                other_info = data_meta.other_info or {}
                ws.cell(**self._meta_cursor).value = \
                    DataMeta.OtherInfoHelper.subject_representation(other_info.get('subject'))
            else:
                try:
                ...
                if t.is_table:
                    ...
                    for row in rows:
                        for header in field_meta.sub_fields:
                            hyperlink = _add_field(header, getattr(row, header.field_name, None),
                                                   fillings_at_start=fillings_at_start)
                            hyperlinks.append(hyperlink)
                    if len(hyperlinks):
                        # 只需要第一个元素超链接
                        value = '=HYPERLINK("{}", "{}")'.format(hyperlinks[0], ugettext("Click to view"))
                        cell.style = 'Hyperlink'
                elif t.is_container or t.is_generator:
                    if isinstance(field_data, dict):
                    ...
                    container_content = DataFieldContainer.objects(id=container_id).first()
                    if container_content:
                        for sub_field in field_meta.sub_fields:
                            _add_field(sub_field, getattr(container_content, sub_field.field_name, None),
                                       fillings_at_start=fillings_at_start)
                elif t.is_array:
                    hyperlinks = []
        ...
        for field in self._template.fields:
            _add_field(field, getattr(content, field.field_name, None))
    ...
    def read_data(self, fp_or_path, uploaded_by, file_dir=None,
            ...
            for meta_row in ws.iter_rows(min_row=2):
                ...
                for index, field in enumerate(DataMetaFieldEnum.reader_iterator()):
                    ...
                    if field == DataMetaFieldEnum.SOURCE:
                        source = value
                        if source is None:
                            raise ParsingError(ParsingErrorEnum.VALUE_MISSING, o1=field.description)
                        methods = []
                        for method in DataMeta.SourceHelper.METHOD_TUPLE:
                            base += 1
                            value = meta_row[index + base].value
                            if value is None:
                                raise ParsingError(ParsingErrorEnum.VALUE_MISSING, o1=(ugettext(method)))
                            if value == ugettext('Yes'):
                                methods.append(ugettext(method))
                            elif value != ugettext('No'):
                                raise ParsingError(ParsingErrorEnum.INVALID_VALUE, o1=value, o2=(ugettext(method)),
                                                   o3=field.description)
                        temp_meta[field.field_raw_name] = \
                            DataMeta.SourceHelper(source, methods).representation()
                    elif field == DataMetaFieldEnum.OTHER_INFO:
                        value = meta_row[index + base].value
                        temp_meta[field.field_raw_name] = {}
                        temp_meta[field.field_raw_name]['subject'] = field.validate(value)
                    elif field == DataMetaFieldEnum.PUBLIC_DATE:
                        try:
                            value = value[0]
                        except Exception:
                            raise ParsingError(ParsingErrorEnum.VALUE_MISSING, o1="公开时间缺失")
                        temp_meta[field.field_raw_name] = field.validate(int(value))
                    elif field == DataMetaFieldEnum.PUBLIC_RANGE:
                        if value is None:
                            raise ParsingError(ParsingErrorEnum.VALUE_MISSING, o1="公开范围缺失")
                        found = False
                        for option in DataPublicRange.DataPublicRangeEnum:
                            if value == DataPublicRange.public_range_en[option] or value == \
                                    DataPublicRange.public_range_zh[option]:
                                temp_meta[field.field_raw_name] = field.validate(int(option))
                                found = True
                                break
                        if not found:
                            raise ParsingError(ParsingErrorEnum.INVALID_VALUE, o1='公开范围字段值不正确')
                    else:
                        if value is None:
        ...
        try:
            ...
            try:
                r = re.search(r'^[1-9]\d*', template_id)
                template_id = r.group()
                if template_id == '':
                    raise TypeError()
    ...
    def generate_template(self, number_of_data=3, elements_per_array=1, rows_per_table=1):
        ...
        for data_count in range(1, number_of_data + 1):
            ...
            for index, field in enumerate(DataMetaFieldEnum.reader_iterator()):
                ...
                if field == DataMetaFieldEnum.SOURCE:
                    ws.cell(**self._meta_cursor).value = ugettext(DataMeta.SourceHelper.SOURCE_TUPLE[0])
                    for _ in DataMeta.SourceHelper.METHOD_TUPLE:
                        self._meta_cursor.move_right()
                        ws.cell(**self._meta_cursor).value = ugettext('No')
                elif field == DataMetaFieldEnum.OTHER_INFO:
                    ws.cell(**self._meta_cursor).value = DataMeta.OtherInfoHelper.subject_representation()
                elif field == DataMetaFieldEnum.PUBLIC_DATE:
                    ws.cell(**self._meta_cursor).value = f'{DataPublicRange.public_time[0]}年后公开'
                elif field == DataMetaFieldEnum.PUBLIC_RANGE:
                    ws.cell(**self._meta_cursor).value = DataPublicRange.public_range_zh[0]
                else:
                    ws.cell(**self._meta_cursor).value = ugettext('Enter %(field_name)s here') % {
                        'field_name': field.description}
                self._meta_cursor.move_right()
            ...
            def _add_field(field_meta: TemplateField, fillings_at_start=(data_id,)):
                ...
                if t.is_table:
                    ...
                    text = ugettext(
                        '=HYPERLINK("#%(sheet_name)s!A1", "Fill sheet ""%(sheet_name)s"" instead of this sheet")')
                    value = text % {
