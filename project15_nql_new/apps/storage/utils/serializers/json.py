...
from .common import *
from apps.storage.models.data import DataMetaFieldEnum, DataMeta
...
from ...models import DataContentImage, DataContentFile
...
class JSONFieldSerializer(FieldSerializer):
    ...
    def get_spec_field_values_by_field_path(self, data_meta: DataMeta, field_path_str: str):
        """
        通过field_path_str获取data_meta中的mongo数据,返回一个字典dict
        Args:
            data_meta: 元数据
            field_path_str: field路径
            with_none: 构造过程中的空值是否返回
        Returns:
            Dict[field_path_str:values]
            field_path_str: field路径
            values:对应field的values,可能是dict也可能是list
            TODO:对values进行进一步确定优化
        """

        def get_value(field_meta: TemplateField, mongo_value, field_path_list: List, field_path_index=1):
            sub_field_path = '.'.join(field_path_list[0:field_path_index])
            if mongo_value is None:
                return None
            t = field_meta.field_type
            if t.is_table:
                rows = DataFieldTableRow.objects(_owner_id=mongo_value)
                field_path_index = field_path_index + 1
                sub_field_path = '.'.join(field_path_list[0:field_path_index])
                for row in rows:
                    for header in field_meta.sub_fields:
                        if header.field_path_str == sub_field_path:  # 下一级路径
                            value = get_value(header, getattr(row, header.field_name, None), field_path_list,
                                              field_path_index + 1)
                            return value
                ...
                for sub_field in field_meta.sub_fields:
                    if sub_field.field_path_str == sub_field_path:  # 下一级路径
                        if sub_field.field_name in container_content:
                            value = get_value(sub_field, getattr(container_content, sub_field.field_name, None),
                                              field_path_list, field_path_index + 1)
                            return value
            elif t.is_array:
                values = []
                if len(mongo_value) == 1:
                    value = get_value(field_meta.element_field, mongo_value[0], field_path_list, field_path_index)
                    return value
                for element_value in mongo_value:
                    value = get_value(field_meta.element_field, element_value, field_path_list, field_path_index)
                    if value is not None:
                        values.append(value)
                return values
            else:
                values = self.serialize_field(mongo_value, field_meta, data_meta.id)
                return values

        template = data_meta.template
        content = DataContent.objects.get(pk=data_meta.dc_id)
        field_path = field_path_str.split('.')
        top_field_path = field_path[0]
        for top_field in template.fields:
            if top_field.field_path_str == top_field_path:
                values = get_value(top_field, getattr(content, top_field_path, None), field_path)
                return {field_path_str: values}

    def get_spec_field_type_values(self, data_meta: DataMeta, field_type: TemplateFieldEnum) -> List[Dict]:
        """
        获取data_meta下指定TemplateFieldEnum的所有数据,返回一个列表list
        即返回get_spec_field_values_by_field_path的列表形式
        Args:
            data_meta:元数据
            field_type:模板字段枚举
        Returns:
            List[Dict[field_path_str:values]]
            field_path_str: field路径
            values:对应field的values,可能是dict也可能是list
        """
        if not field_type.is_basic_or_attachment:
            return []
        template = data_meta.template
        template_path_str_field_map = template.find_spec_field_paths(field_type)
        field_path_str_value_map_list = []
        for field_path_str in template_path_str_field_map.keys():
            field_path_str_value_map_list.append(self.get_spec_field_values_by_field_path(data_meta, field_path_str))
        return field_path_str_value_map_list
...
class JSONHandler(DataHandler):
    ...
    def _data_to_dict(self, data_meta: DataMeta, with_none, absolute_file_path, decompose_range, to_url):
        ...
        def _add_field(field_meta: TemplateField, mongo_value):
            if mongo_value is None:
                return None
            t = field_meta.field_type
            if t.is_table:
                ...
                values = self._serializer.serialize_field(mongo_value, field_meta, data_meta.id)
            return values
        ...
        for field in self._template.fields:
            v = _add_field(field, getattr(content, field.field_name, None))
            if v is not None or with_none:
    ...
    def read_data(self, path_or_fp, uploaded_by: str, file_dir=None,
        ...
        try:
            ...
            e.add_detail(f" (数据ID：{current_meta_id}, 元素序号：{current_meta_index})")
            raise e
        except Exception:
            self.roll_back()
            raise
    ...
    def generate_template(self, number_of_data=2, elements_per_array=2, rows_per_table=2):
        ...
        for index in range(1, number_of_data + 1):
            ...
            for field in DataMetaFieldEnum.reader_iterator(with_id=True):
                ...
                elif field == DataMetaFieldEnum.SOURCE:
                    text = 'MGE-SOURCE_HEADER v1 0000 10 #'
                elif field == DataMetaFieldEnum.OTHER_INFO:
                    text = 'project: 2016YFB0700500；subject: 2016YFB0700503'
                else:
