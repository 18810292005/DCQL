...
@ensure_csrf_cookie
def my_templates(request: HttpRequest, state: str = "all", page: int = 1):
    q = {}
    if state == "all":
        q = {'user': request.user}
    else:
        try:
            ...
            raise MGEError.NOT_FOUND
    paginator = Paginator(Template.objects.filter(**q), 10)
    try:
        templates = paginator.page(page)
    except EmptyPage:
        raise MGEError.NOT_FOUND
    else:
        return render(request, template_name='account/my-templates.html',
                      context={'template_list': templates, 'page_count': paginator.num_pages,
                               'current_page': page, 'state': state})


@require_role(UserRole.DATA_UPLOADER | UserRole.DATA_ADMIN, is_api=False)
...
def admin_user_list(request: HttpRequest):
    q = request.GET.get('q', "")
    page = request.GET.get('page', 1)
    if q:
        q_object = Q(username__icontains=q) | Q(real_name__icontains=q) | Q(institution__icontains=q) | Q(
            email__icontains=q)
    else:
        q_object = Q()  # FIXME _system用户
    users = User.objects.filter(q_object).order_by('-date_joined')
    ...
    try:
        ...
        result_page = paginator.page(1)
        page = 1
    context = {'user_list': result_page, 'page_count': paginator.num_pages, 'current_page': page, 'query_string': q}
    return render(request, 'account/user_list.html', context=context)


@require_role(UserRole.USER_ADMIN, is_api=False)
def user_permissions(request: HttpRequest, username):
    try:
        user = User.objects.get(username=username)
    except User.DoesNotExist:
        raise MGEError.NOT_FOUND
    else:
        roles = [{'role_name': x.description, 'role_value': x.value, 'has_role': user.roles & x.value} for x in
                 UserRole.settable_roles()]
        categories = MaterialCategory.get_leaves()
        granted_categories = [x['category_id'] for x in
                              AdminMaterialCategory.objects.filter(user=user).values('category_id')]
        return render(request, 'account/user_permissions.html',
                      context={'user': user, 'roles': roles, 'current_roles': user.roles, 'categories': categories})
