...
class QueryFactory:
    ...
    def _produce(query: dict) -> dict:
        ...
        if 'field' in query and 'op' in query and 'val' in query:
            if query['op'] in simpleField_operator:
                return simpleField_operator[query['op']](query['field'],
                                                         query.get('val'))
        ...
        if 'field' in query and 'op' in query:
            if query['op'] in singleField_operator:
                return singleField_operator[query['op']](query['field'])
    ...
    @staticmethod
    def _produce_fulltext(query: dict):
        if query is None:
            return None
        return QueryFactory.to_dsl(
            Or(queries=[
                MatchPhrase(field_path='summary', value=query),
...
class MultiTemplateQueryFactory(QueryFactory):
    ...
    def _produce_data(query: dict):
        ...
        query_list = []
        for v in query:
            if v.get("q", None) is not None:
                query_list.append(And(queries=[QueryFactory._produce(v['q']),
                                               Equal(field_path='template', value=int(v['tid']), force_prefix=())]))
            else:
                query_list.append(Equal(field_path='template', value=int(v['tid']), force_prefix=()))
