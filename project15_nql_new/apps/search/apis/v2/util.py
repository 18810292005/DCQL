...
def extract_category_and_template(aggregations, value):
    ...
    for template in aggregations['template']['buckets']:
...
def extract_summary(aggregations, value):
    ...
    return dict(
        ...
        realname=aggregations['realname']['buckets'],
        template=aggregations['template']['buckets'],
        subject=aggregations['subject']['buckets'],
        project=aggregations['project']['buckets'],
        source=aggregations['source']['buckets'],
        group_by_time=aggregations['group_by_time']['buckets'],
        keywords=aggregations['keywords']['buckets'],
    )
...
def extract_params_from_request(request):
    ...
    if len(return_meta_fields) == 0:
        return_meta_fields = ['id', 'template', 'template_name', 'category', 'category_name',
                              'abstract', 'doi', 'source', 'methods', 'subject', 'project', 'reference',
                              'title', 'add_time', 'user', 'realname',
                              'views', 'downloads', 'score', 'external_link']
    for field in ('id', 'template'):
        if field not in return_meta_fields:
            return_meta_fields.append(field)

    return_data_fields = extract_source_fields(request, 'return_data[]')
    return_fields = return_meta_fields + ['content.' + field for field in return_data_fields]
