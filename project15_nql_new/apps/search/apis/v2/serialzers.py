...
from apps.search.apis.v2.util import extract_summary, extract_params_from_request, extract_result
...
class QueryField(serializers.Field):
    ...
    def to_representation(self, value):
        try:
            ...
            results = [extract_result(hit, value) for hit in es_query['hits']['hits']]
...
class DownloadField(serializers.Field):
    ...
    def to_representation(self, value):
        ...
        data_include = [dict(id=ins.id, title=ins.title, template=ins.template_id, template_title=ins.template.title) for ins in data_include]

        data_exclude = DataMeta.objects.filter(id__in=value['data']['exclude'])
        data_exclude = [dict(id=ins.id, title=ins.title, template=ins.template_id, template_title=ins.template.title) for ins in data_exclude]
