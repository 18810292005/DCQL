...
action                             : CREATE | DROP | ALTER | DESCRIBE | INSERT | DELETE | UPDATE | SELECT |
                                     RENAME | SHOW | ALL ;
resource                           : TEMPLATE tableName? | TEMPLATES | ROLE role_name? | ROLES (WITH GRANT)? |
...
//alterStatement
alterStatement       : ALTER TEMPLATE tableName (',' tableName)? alterStatements (',' alterStatements)* ;
...
updateStatement      : UPDATE tableName SET column_name '=' value whereClause? ;
deleteStatement      : DELETE FROM tableName whereClause? ;
dropStatement:
    DROP TEMPLATE tableName
;
//insertStatement
insertStatement:
...
tableSources
    : tableName |  tableName '(' column_name ')' (',' tableName '(' column_name ')')+
    ;
