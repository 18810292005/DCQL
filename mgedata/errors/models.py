        ...
        for member, value in class_dict.items():
            if isinstance(value, MGEException):
                if value.code in _MGEErrorMetaClass._error_code_dict:
                    raise ValueError(
                        f"错误码{value.code}已经存在({_MGEErrorMetaClass._error_code_dict[value.code][0]})")
...
class MGEError(metaclass=_MGEErrorMetaClass):
    ...
    UNAUTHORIZED = e(_("Unauthorized"), 401, 200)
    USER_DISABLED = e(_("用户已禁用"), 402, 200)
    NOT_FOUND = e(_("未找到"), 404, 404)
    BAD_TOKEN = e(_("Invalid token"), 2002, 401)
    USER_NOT_FOUND = e(_("用户不存在"), 2003, 200)
    WRONG_PASSWORD = e(_("密码错误"), 2004, 200)
    USER_ALREADY_EXISTS = e(_("用户名已存在"), 2005, 403)
    ...
    BAD_IMAGE = e(_("不是有效的图片文件"), 2012, 422)
    ...
    NO_VALID_FILE = e(_("找不到任何xlsx或json文件。如果使用压缩包上传，请确保文件位于压缩包的最外层。"), 2108, 422)
    ...
    SUMMIT_PART_ERROR = e(_("提交数据部分出错"), 2113, 422)
    SUMMIT_ERROR = e(_("提交数据出错"), 2114, 422)
    ...
    WRONG_PASSWORD_FORMAT = e(_("密码格式不正确"), 7006, 422)
    PARSE_FAILED = e(_("解析失败"), 7007, 422)
    DATA_INCONSISTENCY = e(_("数据不一致"), 10001, 422)
