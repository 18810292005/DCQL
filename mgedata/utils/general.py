...
def json_response(data=None, msg: str = '', status_code: int = 200, **kwargs) -> HttpResponse:
    ...
    :return: None
    """
    result = {'code': MGEError.SUCCESS.code,
              'data': data}
    if msg:
        result['msg'] = msg
    if kwargs:
...
def load_request_body(request=None, expected=dict):
    ...
    try:
        ...
        if request.content_type != 'application/json':
            raise MGEError.NOT_ACCEPTABLE('Content-Type must be application/json')
        json_data = json.loads(request.body)
        if not isinstance(json_data, expected):
            raise MGEError.NOT_ACCEPTABLE('expect %s but %s given' % (str(expected), str(type(json_data))))
        return json_data
    except Exception as ex:
        raise MGEError.NOT_ACCEPTABLE(str(ex))
...
def get_field(json_data, field, required_type=object, allow_none=False, allowed=None, force=False, default=None):
    ...
    if value is None and not allow_none:
        if not force:
            raise MGEError.FIELD_MISSING(_('field "%(field_name)s" is missing') % {'field_name': field})
        ...
        raise MGEError.WRONG_FIELD_TYPE(_('Field "%(field)s" should be %(required_type)s.') % {
...
def get_json_field_r(request: HttpRequest, field, required_type=object, allow_none=False, allowed=None, force=False,
    ...
    if hasattr(request, attr):
        ...
        try:
            ...
            if not force:
                raise MGEError.NOT_ACCEPTABLE(str(e))
            return default
...
def get_multipart_field(request: HttpRequest, field, required_type=str, allow_none=False, allowed_values=None,
    ...
    if not request.content_type.startswith('multipart/form-data'):
        raise MGEError.NOT_ACCEPTABLE("Content-Type必须为multipart/form-data")
...
def get_param_value(field, value, allow_none=True, allowed=None, convert_to=None, force=False):
    ...
    if value is None and not allow_none:
        if not force:
            raise MGEError.FIELD_MISSING('Field "%s" is missing' % field)
        return None
    elif value is None and allow_none:
        return None
    elif allowed is not None and value not in allowed:
        if not force:
            raise MGEError.WRONG_FIELD_TYPE(_('Value of field "%(field)s" should be one of %(allowed_values)s.') % {
    ...
    def decorator(func):
        ...
        def inner(request, *args, **kwargs):
            if request.method not in request_methods_list:
                raise MGEError.METHOD_NOT_ALLOWED('Method ' + request.method + ' is not allowed!')
