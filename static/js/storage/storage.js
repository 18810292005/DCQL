 ...
 */
var DUMMY_TEXT = [
  gettext("STRING"), gettext("NUMBER"), gettext("RANGE"),
  gettext("IMAGE"), gettext("FILE"), gettext("ARRAY"), gettext("CHOICE"),
  gettext("TABLE"), gettext("CONTAINER"), gettext("GENERATOR")];
// 各类工具函数
var Utils = {
  /**
   * 删除 that 对象的 parent
...
var TemplateFactory = {
  _CHECK_FORM: doT.compile('<div class="checkbox"><label><input type="checkbox" name="{{=it.name}}" {{?it.chk}}checked{{?}}>{{=it.label}}</label></div>'),
  _CONTEXTUAL_SCOPE: doT.compile('<p class="bg-{{=it.type||\'info\'}}">{{=it.message}}{{?!it.stay}}<button type="button" class="close" aria-label="Close" onclick="Utils.removeP(this)"><span aria-hidden="true">&times;</span></button>{{?}}</p>'),
  _CHOICE_SCOPE: doT.compile('<ul><li class="tools"><div class="col-xm-3 col-xs-3"><input class="form-control input-sm" onkeyup="Utils.appendOptionByKeyUp(event,this)" placeholder="' + gettext('Press Enter to add option') + '"></div><button class="btn btn-sm btn-default" onclick="Utils.appendOptionByClick(this)">' + gettext('Add option') + '</button>{{?it.addGroup}}<button class="btn btn-sm btn-default" onclick="Utils.appendGroup(this)">' + gettext('Add group') + '</button>{{?}}</li></ul>'),
  _CHOICE_LI: doT.compile('<li class="{{=it.type}}">{{=it.name}}<i class="fa fa-remove" onclick="Utils.removeP(this)"></i></li>'),
  ...
  _wrapTFieldRow: function (type, inlineCtt, extraCtt, n, r) {
    var $field = $(FormFieldFactory._ROW_WRAP).addClass('t-field'),
      ...
      moveDownIcon = '<i class="fa fa-chevron-down" title="' + gettext('Down') + '" onclick="Utils.moveRowDown(this)"></i>',
      moveUpIcon = '<i class="fa fa-chevron-up" title="' + gettext('Up') + '" onclick="Utils.moveRowUp(this)"></i>',
  ...
  getArray: function (n, r, field) {
    ...
    var $inlineCtt = $fgSel.add('<button type="button" class="btn btn-sm btn-default" onclick="Utils.setArrayType(this)">' + gettext('Set this type') + '</button>');
  ...
  getTable: function (n, r, head, headType) {
    ...
    var $inlineCtt = $fgSel.add('<button type="button" class="btn btn-sm btn-default" onclick="Utils.addTableCol(this)">' + gettext('Add column') + '</button>');
  ...
  getGenerator: function (n, r, opts, optVals) {
    ...
    var $inlineCtt = $fgSel.add('<button type="button" class="btn btn-sm btn-default" onclick="Utils.addTableCol(this)">' + gettext('Add field') + '</button>');
    var $tfcExtra = $('<div class="t-field-content-extra"></div>');
...
var FormFieldFactory = {
  ...
  _DELETE_ICON: doT.compile('<i class="fa fa-remove" title="{{=it.title||gettext(\'Delete\')}}" onclick="{{=it.clk||\'Utils.removeP(this)\'}}"></i>'),
  // eg: {lb: 'label'} (required)
  _FORM_LABEL: doT.compile('<label class="control-label">{{=it.lb}}</label>'),
  // eg: {lg: 6, md: 6, sm: 9, xs: 12} (default)
  _FORM_WIDTH_WRAP: doT.compile('<div class="col-lg-{{=it.lg||6}} col-md-{{=it.md||6}} col-sm-{{=it.sm||9}} col-xs-{{=it.xs||12}}"></div>'),
  ...
  _TEXT_INPUT: doT.compile('<input class="form-control" name="{{=it.name||\'\'}}" value="{{=it.val||\'\'}}" placeholder="{{=it.plh||\'\'}}">'),
  // eg: {name: 'name', val: 5, plh: 'e e e', max: 10, min: 1, step: 2}  (name, plh default to null string, max, min, step not required)
  _NUMBER_INPUT: doT.compile('<input type="number" class="form-control" name="{{=it.name||\'\'}}" value="{{=it.val||\'\'}}" placeholder="{{=it.plh||\'\'}}" {{?it.max}}max="{{=it.max}}"{{?}} {{?it.min}}min="{{it.min}}"{{?}} {{?it.step}}step="{{=it.step}}"{{?}}>'),
  ...
  _SELECT_INPUT2: doT.compile('<select name="{{=it.name||\'\'}}" class="form-control">{{~it.opts :opt}}<option value="{{=opt.val}}" {{?it.sel === opt.val}}selected{{?}}>{{=opt.name}}</option>{{~}}{{~it.grps :grp}}<optgroup label="{{=grp.name}}">{{~grp.items :item}}<option value="{{=item.val}}" {{?it.sel === item.val}}selected{{?}}>{{=item.name}}</option>{{~}}</optgroup>{{~}}</select>'),
  ...
  _SELECT_INPUT: doT.compile('<select name="{{=it.name||\'\'}}" class="form-control" {{?it.chg}}onchange="{{=it.chg}}"{{?}}>{{~it.opts :opt}}<option {{?it.sel === opt}}selected{{?}}>{{=opt}}</option>{{~}}{{~it.grps :grp}}<optgroup label="{{=grp.name}}">{{~grp.items :item}}<option {{?it.sel === item}}selected{{?}}>{{=item}}</option>{{~}}</optgroup>{{~}}</select>'),
  // eg: {name: 'name', multi: false, acc: 'image/*', chg: 'Utils.uploadFile(this)'}  (default accept *)
  _FILE_INPUT: doT.compile('<input type="file" class="form-control" name="{{=it.name||\'\'}}" {{?it.multi}}multiple{{?}} accept="{{=it.acc||\'\'}}" {{?it.chg}}onchange="{{=it.chg}}"{{?}}>'),
  ...
  _getRangeInputGroup: function (name, r, rangeType, unit, val1, val2) {
    ...
    $group.find('span:first').after(this._NUMBER_INPUT({
      val: val2,
      plh: plh2 + (r ? ("(" + gettext('Required') + ")") : '')
    }));
    return $group;
  },
  // 包裹每个字段的内容
  ...
  getString: function (name, r, val) {
    var input = FormFieldFactory._TEXT_INPUT({
      name: name,
      val: val,
      plh: name + (r ? ("(" + gettext('Required') + ")") : '')
    });
  ...
  getNumber: function (name, r, unit, val) {
    var input = FormFieldFactory._NUMBER_INPUT({
      name: name,
      val: val,
      plh: name + (r ? ("(" + gettext('Required') + ")") : '')
    });
  ...
  getTable: function (name, r, head, headType, rows) {
    ...
    head.forEach(function (hName) {
      ...
      switch (headType[hName].t) {
        case FieldTypeEnum.STRING:
          $fg.append(FormFieldFactory._TEXT_INPUT({
            name: hName,
            plh: hName + (headR ? ("(" + gettext('Required') + ")") : '')
          }));
          break;
        case FieldTypeEnum.NUMBER:
          var input = FormFieldFactory._NUMBER_INPUT({
